...
import Steganography.text_to_text as ttt
...
"}")
    ...
    def encode_call_back(self):
        if self.fromComboBox.currentText() == "Text":
            ...
            elif self.toComboBox.currentText() == "Text":
                self.message = self.inputLineEdit.text()
                self.file_path = pathlib.Path(self.toLineEdit.file_path_name)
                self.text_to_text = ttt.EncodeTextSteg(self.file_path)
                self.text_to_text.encodeIntoText(self.message)
        else:
            pass
