...
import Steganography.text_from_text as tft
...
"}")
    ...
    def decode_call_back(self):
        if self.decodeComboBox.currentText() == "Text":
            ...
            elif self.input_audio_flag == True:
                self.audio_to_text = att.AudioToText()
                self.msg = self.audio_to_text.decode(self.file_path)

            elif self.input_text_flag == True:
                self.text_from_text = tft.TextSteganography(self.file_path)
                self.msg = self.text_from_text.decodeFromText()
                print("The encoded message is {}".format(self.msg))
            else:
                pass
